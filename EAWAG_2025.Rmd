---
title: "R-scripts4Schaefer et al. 2025"
...
#setwd("/Users/shuqing.xu/Seafile/XDrive/Projects/EAWAG/Revision/")
...
Data.all<- sapply(readxl::excel_sheets("./data/ADD-Monitoring_v3.xlsx"), simplify = F, USE.NAMES = F,
                  function(X) readxl::read_excel("./data/ADD-Monitoring_v3.xlsx", sheet = X))
...
AphidEffects(data=((ADD.Monitor.all.sub.clean$NP_ratio)),Block,Pond=ADD.Monitor.all.sub.clean$Pond,Time =  ADD.Monitor.all.sub.clean$Time,Week=ADD.Monitor.all.sub.clean$Week, Year=ADD.Monitor.all.sub.clean$Year, Treatment = ADD.Monitor.all.sub.clean$Treatment,TimeRemove = 2,Parameters  = "NP_ratio")
...
AphidEffects(data=(log1p(ADD.Monitor.all.sub$AphidAbundance)),Block,Pond=ADD.Monitor.all.sub$Pond,Time =  ADD.Monitor.all.sub$Time,Week=ADD.Monitor.all.sub$Week, Year=ADD.Monitor.all.sub$Year, Treatment = ADD.Monitor.all.sub$Treatment,TimeRemove = 0,Parameters  = "AphidAbundance")
...
## Analyze resistotype data and create Figure S15
...
## Analzye population genomics data and create figrue 2, S16, S17
...
info <- read.table("./data/SequenceID_info.txt", header=TRUE, sep="\t",
                   stringsAsFactors=FALSE)

# Keep only the samples we want (remove those flagged "Y", and only 2021 & 2022)
#info <- subset(info, Remove=="N" & Year %in% c(2021,2022))

# 2. Define the path to your VCF and the (haploid) pool sizes:
vcf_file    <- "./data/final.dp20_400.recode.vcf.gz"
# TODO: replace the following with your true haploid pool sizes per sample:
pool_sizes  <- rep(250, nrow(info))


# 3. Read the VCF into a single pooldata object,
#    tuning blocks and threads for large files :contentReference[oaicite:0]{index=0}
pd_all <- vcf2pooldata(vcf.file = "./data/final.dp20_400.recode.vcf.gz", 
                                poolsizes =pool_sizes ,
                                poolnames = info$Sample, min.cov.per.pool =20,
                                nlines.per.readblock = 1e+06, 
                                verbose = TRUE )
# Helper to subset a pooldata object by sample names

# 4. Compute all pairwise FST at once
pw <-compute.fstats(pd_all, computeQmat = F,
                                   nsnp.per.bjack.block = 100,computeF4 = F,
                                   verbose = TRUE)
fst_mat <- slot(pw, "pairwise.fst")   # matrix of Fst estimates :contentReference[oaicite:0]{index=0}

info <- subset(info, Remove=="N")



# 4) Define sample‐sets for each category
s_init  <- info$Sample[info$Year=="Initial"]
s_ctl21 <- info$Sample[info$Year=="2021" & info$Treatment=="Control"]
s_aph21 <- info$Sample[info$Year=="2021" & info$Treatment=="Aphid"]
s_ctl22 <- info$Sample[info$Year=="2022" & info$Treatment=="Control"]
s_aph22 <- info$Sample[info$Year=="2022" & info$Treatment=="Aphid"]
all21   <- info$Sample[info$Year=="2021"]
all22   <- info$Sample[info$Year=="2022"]

groups <- list(
  WithinTreatment_0_0        = combn(s_init, 2, simplify=FALSE),
  WithinControl_2021_2021    = combn(s_ctl21, 2, simplify=FALSE),
  WithinControl_2022_2022    = combn(s_ctl22, 2, simplify=FALSE),
  WithinAphid_2021_2021      = combn(s_aph21, 2, simplify=FALSE),
  WithinAphid_2022_2022      = combn(s_aph22, 2, simplify=FALSE),
  BetweenTreatment_2021_2021 = expand.grid(A=s_aph21, B=s_ctl21,
                                           stringsAsFactors=FALSE),
  BetweenTreatment_2022_2022 = expand.grid(A=s_aph22, B=s_ctl22,
                                           stringsAsFactors=FALSE),
  BetweenYear_0_2021         = expand.grid(A=s_init,  B=all21,
                                           stringsAsFactors=FALSE),
  BetweenYear_0_2022         = expand.grid(A=s_init,  B=all22,
                                           stringsAsFactors=FALSE),
  BetweenYear_2021_2022      = expand.grid(A=all21,   B=all22,
                                           stringsAsFactors=FALSE)
)

# helper to extract numeric Fst values from a 2‐col df of pairs
get_vals <- function(df_pairs) {
  fst_mat[cbind(df_pairs[[1]], df_pairs[[2]])]
}

# 5) Loop through groups → compute mean, SE, 99% CI
z99   <- qnorm(0.995)
stats <- do.call(rbind, lapply(names(groups), function(g) {
  lst <- groups[[g]]
  
  # turn combn‐list into a 2‐col data.frame
  if (!inherits(lst, "data.frame")) {
    df_pairs <- do.call(rbind, lapply(lst, function(x)
      data.frame(A = x[1], B = x[2], stringsAsFactors=FALSE)))
  } else {
    df_pairs <- lst
  }
  
  vals <- get_vals(df_pairs)
  m    <- mean(vals)
...
# 6) Barplot of Mean ± SE
data.poolstat.summary.plot<-ggplot(stats, aes(x=Group, y=Mean_Fst)) +
  geom_bar(stat="identity") +
  geom_errorbar(aes(ymin=Mean_Fst - SE, ymax=Mean_Fst + SE),
                width=0.2) +
  theme_bw() +
  theme(
  ...
  labs(x="", y=expression(F[ST]~"Mean ± SE"))
...
colnames(pw@pairwise.fst)
row.names(pw@pairwise.fst)
...
fst.hm <- Heatmap(pw@pairwise.fst,
...
```
## Analzye transplant experiment for D. magna and create Figure 3
...
## Analzye transplant experiment of the duckweed and aphids and make Figure 4
